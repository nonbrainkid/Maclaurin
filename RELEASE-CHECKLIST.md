...
| Тестнет | Все четыре контракта развёрнуты заново под новыми именами и раскладка genesis выполнена — адреса и что осталось в §3.4. **Ни один из них не верифицирован**, поэтому наружу они не идут. Адреса из `LAUNCH-PLAN.md` этап 2 — прошлая версия с тикером `TAYLOR`, ссылаться на них нельзя |
| Git | Первый коммит есть (`db3ef18`), так что тег §1.4 стало на что вешать |
...
|---|---|
| Тега на мейннет-коммите (сама история уже не пуста — есть `db3ef18`) | §1.4 |
| Аудита `Launch.s.sol` и диффа переименования — сами контракты фазы 4 аудит прошли дважды | §2.5 |
| Записи о фактическом прогоне halmos: доказательства написаны, но не прогнаны до конца | §2.4 |
| Репетиций `Launch.s.sol` на anvil и на форке мейннета (в тестнете он уже прогнан) | §3.2, §3.3 |
| Верификации тестнет-контрактов на Blockscout — ни одного из четырёх | §3.4 |
| Ручной проверки кривой деньгами в тестнете: покупка, продажа, `ExceedsPurchased`, `AntiSnipeLimit` | §3.4 |
...
`DEPLOY-RUNBOOK.md` §12 и **уже прогнан в тестнете**: казна и кривая развёрнуты,
раскладка genesis выполнена, на адресе создателя ноль. Осталось прогнать его на anvil
и на форке мейннета — и провести аудит самого скрипта.
...
**Что осталось:** прогон в тестнете состоялся (§3.4) — казна и кривая развёрнуты,
раскладка выполнена. Не хватает репетиций на anvil и на форке мейннета и аудита самого
скрипта: он написан позже последнего аудиторского прохода.
...
- **Сделано:** прогон на форке мейннета отправляет 8 транзакций (2 деплоя + 6 переводов),
  ...
  all post-checks hold`; тот же прогон повторён на anvil (§3.2), а в тестнете (§3.4)
  уже повторён.
...
- [ ] **верификация всех четырёх контрактов на тестнет-Blockscout** — ⛔ **не пройдена**:
      API возвращает пустой `SourceCode`. Это репетиция §6, и пока она не пройдена,
      команды верификации в мейннете не проверены ничем;
- [ ] реальная покупка и продажа обратно тестовым ETH, включая продажу **чужим**
      адресом → `ExceedsPurchased` (та самая дыра, которую закрыл `boughtOf`, —
      проверить её в живой сети руками, а не только тестом);
- [ ] попытка купить больше `ANTI_SNIPE_MAX` в первый час → `AntiSnipeLimit`;
- [ ] `release()` казны до срока → `StillLocked`;
- [ ] `verify()` раскладки:
      `forge script script/Distribute.s.sol --sig "verify()" --rpc-url rh_testnet`.
- **Сделано:** все пункты отмечены, ничего не зареверило неожиданно, а всё, что должно
  было зареверить, зареверило **с правильной ошибкой**.
...
Порядок из трёх попыток (в **первом** тестнет-прогоне, до переименования, сработала
попытка 2 с `--constructor-args` — вопреки пометке «Only for Etherscan» в справке forge.
На новых тестнет-адресах верификация ещё не повторялась, см. §3.4):
...
2. **README репозитория:** адреса всех четырёх контрактов со ссылками на Blockscout,
   ссылка на транзакцию сжигания, обновлённый статус проверок (тесты/Slither/halmos/аудиты).
   Механика фаз 1–4, раскладка genesis и оговорка про риски в README уже описаны;
   после деплоя туда добавляются только адреса и пруфы.
